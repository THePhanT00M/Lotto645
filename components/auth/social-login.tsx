...
/** 구글·애플 소셜 로그인 버튼 묶음. */
export default function SocialLogin({ label, onSelect }: SocialLoginProps) {
  return (
      <>
        <div className="relative py-2">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="text-ink-muted bg-auth-card px-4">{label}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <SocialButton onClick={() => onSelect("apple")}>
            <AppleIcon />
            Apple
          </SocialButton>
        </div>
...
function SocialButton({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
      <Button
          ...
          variant="outline"
          onClick={onClick}
          className="border-line text-ink h-11 rounded-lg bg-white transition-colors hover:bg-gray-50 dark:bg-[#121212] dark:text-white dark:hover:bg-[#2a2a2a]"
      >
        {children}
      </Button>
  )
