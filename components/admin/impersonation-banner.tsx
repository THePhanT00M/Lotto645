...
export default function ImpersonationBanner({ targetName }: ImpersonationBannerProps) {
  ...
  const leave = async () => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: t.admin.impersonation.failed,
        description: error instanceof Error ? error.message : t.admin.impersonation.relogin,
      })
    }
  }
  ...
  return (
      <div className="flex items-center justify-center gap-3 bg-amber-500 px-4 py-2 text-sm font-medium text-black">
        <span className="flex items-center gap-1.5">
          ...
          {t.admin.impersonation.viewing(targetName)}
        </span>
